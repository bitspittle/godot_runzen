...
var deltas = CircularBuffer.new()

func _process(delta):
	history.append(Input.get_accelerometer())
	if history.size() > 2:
		var recent: Vector3 = history.get_item(-1)
		var prev: Vector3 = history.get_item(-2)

		deltas.append(recent - prev)
...
#
#func _roundf(num: float, digit: int) -> float:
#    return round(num * pow(10.0, digit)) / pow(10.0, digit)
#
#func _round(vec3: Vector3, digit: int = 2) -> Vector3:
#	return Vector3(_roundf(vec3.x, digit), _roundf(vec3.y, digit), _roundf(vec3.z, digit))
#
#func _str2(s: String, digit: int = 2) -> String:
#	var parts = s.split(".")
#
#	if parts.size() == 1:
#		return parts[0] + "." + "0".repeat(digit)
#
#	var dec_len = parts[1].length()
#	if dec_len > digit:
#		return parts[0] + "." + parts[1].substr(0, digit)
#	elif dec_len < digit:
#		return parts[0] + "." + parts[1] + "0".repeat(digit - dec_len)
#	else:
#		return s
#
#func _str(vec3: Vector3, digit: int = 2) -> String:
#	var rounded = _round(vec3)
#	var x = str(rounded.x)
#	var y = str(rounded.y)
#	var z = str(rounded.z)
#
#	return "[" + _str2(x, digit) + ", " + _str2(y, digit) + ", " + _str2(z, digit) + "]"
#
...
#	get_node("Control/Accelerometer").text = 'Accelerometer: ' + _str(acc) + ', gravity: ' + _str(grav)
#	get_node("Control/Magnetometer").text = 'Magnetometer: ' + _str(mag)
#	get_node("Control/Gyroscope").text = 'Gyroscope: ' + _str(gyro)
#
